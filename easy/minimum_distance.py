...
Print a line containing the minimal sum of distances for an optimal sorority
location.
...
def main(dfile):
    ...
    with open(dfile, 'r') as dfld:
        for l in dfld:
            l = l.strip('\n')
            nfriends, *addresses = [int(n) for n in l.split()]
            
            print(min([sum([abs(e - h) for e in addresses])
                           for h in range(min(addresses),
                                          max(addresses) + 1)]))
