...
def main(dfile):
    ...
    with open(dfile, 'r') as dfld:
        for l in dfld:
            line = l.strip('\n')
            ratio_of_uppercase_letters = round(
                reduce(lambda s, c: s + int(c.isupper()), line, 0) \
                * 100.0 / len(line), 2)
            print "lowercase: {:.2f} uppercase: {:.2f}".format( \
                100 - ratio_of_uppercase_letters, ratio_of_uppercase_letters)
